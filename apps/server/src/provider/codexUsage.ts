...
function getCodexHome(deps: CodexUsageDependencies): string {
  if (deps.homePath && deps.homePath.trim().length > 0) {
    return deps.homePath;
  }
...
function windowLabel(windowMinutes: number | undefined, fallback: string): string {
  ...
  if (windowMinutes >= 60 * 24) {
    return `${Math.round(windowMinutes / (60 * 24))}d`;
  }
  if (windowMinutes >= 60) {
...
function toUsageWindow(
  ...
  const label = windowLabel(toNumber(raw.window_minutes), fallbackLabel);
...
export function readCodexUsageFromRollout(
  ...
  const windows = [
    toUsageWindow("primary", "Primary", found.rateLimits.primary),
    toUsageWindow("secondary", "Secondary", found.rateLimits.secondary),
  ].filter((entry): entry is ServerProviderUsageWindow => entry !== undefined);
...
export function resolveCodexUsageSnapshot(
  ...
  const rollout = findLatestRollout(getCodexHome(dependencies));
  if (!rollout) {
    return undefined;
  }
