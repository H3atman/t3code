...
function normalizeUsageState(value: unknown): ServerProviderUsageState | undefined {
...
function formatKeyLabel(key: string): string {
  switch (key.replace(/[\s_-]+/g, "").toLowerCase()) {
    case "fivehour":
    case "5hour":
    case "5h":
      return "5h";
...
function readPercent(record: Record<string, unknown>): number | null {
...
function readWindowLabel(record: Record<string, unknown>, fallbackKey?: string): string {
  const explicit =
...
function hasWindowShape(record: Record<string, unknown>): boolean {
  return (
...
function collectWindowCandidates(
  value: unknown,
): Array<{ readonly key?: string; readonly record: Record<string, unknown> }> {
  ...
  const candidates: Array<{ readonly key?: string; readonly record: Record<string, unknown> }> = [];
...
function normalizeProviderUsageSnapshot(
  value: unknown,
  checkedAt: string,
...
function mergeCodexAccount(
  provider: ServerProvider,
  payload: ProviderRuntimeAccountUpdatedEvent["payload"]["account"],
): ServerProvider {
...
export function mergeProviderRuntimeEventIntoSnapshot(
  provider: ServerProvider,
  event: ProviderRuntimeAccountUpdatedEvent | ProviderRuntimeAccountRateLimitsUpdatedEvent,
): ServerProvider {
...
export function isProviderUsageRuntimeEvent(
  event: ProviderRuntimeEvent,
): event is ProviderRuntimeAccountUpdatedEvent | ProviderRuntimeAccountRateLimitsUpdatedEvent {
  return event.type === "account.updated" || event.type === "account.rate-limits.updated";
}
