...
interface ClaudeUsageApiPayload {
  readonly five_hour?: ClaudeUsageWindowPayload;
  readonly seven_day?: ClaudeUsageWindowPayload;
}
...
function readClaudeOauthCredentials(
  ...
  const oauth = asRecord(
    asRecord(JSON.parse(fs.readFileSync(credentialsPath, "utf8")))?.claudeAiOauth,
  );
...
function readCacheRecord(cachePath: string): ClaudeUsageCacheRecord | undefined {
...
function readUsageCache(cachePath: string, now: number): ServerProvider["usage"] | undefined {
  const parsed = readCacheRecord(cachePath);
  if (!parsed?.usage) {
    return undefined;
  }
...
function buildClaudeWindow(
  id: string,
  label: string,
  raw: ClaudeUsageWindowPayload | undefined,
  ...
  return {
    ...
    percentUsed,
    resetsAt,
    exhausted: percentUsed !== null && percentUsed >= 100,
...
function parseClaudeUsageSnapshot(
  ...
  const response = value as ClaudeUsageApiPayload;
...
export async function resolveClaudeUsageSnapshot(
  ...
  try {
    ...
    if (!result.ok) {
      const previous = readCacheRecord(cachePath);
